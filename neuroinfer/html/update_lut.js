window.updateLUT = function () {
  ...
  for (let i = 0; i < step_grad; i++) {
    ...
    target_value =
      (parseInt(minvalue.value) +
        ((1 - parseInt(minvalue.value)) * i) / step_grad) /
      max_value;
  ...
  for (let i = 0; i < step_grad; i++) {
    ...
    rgbColors = hslToRgb(color_i / 360, 1, 0.5);
    target_value =
      (1 + ((parseInt(maxvalue.value) - 1) * i) / step_grad) / max_value;
    lutData.push([target_value, rgbColors[0], rgbColors[1], rgbColors[2]]);
  }
  ...
  for (let i = 0; i < step_grad; i++) {
    ...
    rgbColors = hslToRgb(color_i / 360, 1, 0.5);
    target_value =
      parseInt(maxvalue.value) / max_value +
      ((1 - parseInt(maxvalue.value) / max_value) * i) / step_grad;
    lutData.push([target_value, rgbColors[0], rgbColors[1], rgbColors[2]]);
  }
  ...
  overlays.forEach(
    (overlay_id) => (params[getFileName(overlay_id)] = { lut: lut_id }),
  );
