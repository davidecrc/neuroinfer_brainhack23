...
def run_bayesian_analysis_coordinates(
    cog_list, prior_list, x_target, y_target, z_target, radius, feature_df, cm
):
    ...
    for q, cog in enumerate(cog_list):
        ...
        if cog not in feature_names:
            ...
            distances = np.linalg.norm(
                dt_papers_nq[["x", "y", "z"]].values - center, axis=1
            )
...
def run_bayesian_analysis_area(cog_list, prior_list, area, radius, feature_df, cm):
    ...
    visited_coord = np.zeros(
        np.max(coordinates_area[0]) - np.min(coordinates_area[0]),
    ...
    for i in range(
        ...
        if ~is_visited((x_target, y_target, z_target), visited_coord):
            ...
            visited_coord = update_visited_coord(
                visited_coord,
                normalize_coord(x_target, y_target, z_target, coord_ranges),
                padding,
            )  # update coordinate visitate+padding
